...
-(void)setStat {
    // (Roger) Setting up the basic elements
...
-(void)setUserInfo {
    ...
    CoreData *core = [CoreData sharedCore];
    CCLabelTTF *username = [CCLabelTTF labelWithString:core.guestUserName fontName:@"Verdana" fontSize:26];
...
- (void) deckChooser {
    ...
    // (Roger) Fetching the data from the deck
    ...
    for (int i = 0; i < deckListCount; i++) {
        ...
        NSString *coverCardName = [[NSString alloc] initWithFormat:@"%d.png", firstCardIndex];
...
- (void)jumpToGameTable: (id) sender {
    NSLog(@"Set up the host player chosen deck");
    CoreData *core = [CoreData sharedCore];
    NSString *deckName = [sender userData];
    core.GuestPlayerDeck = [[NSString alloc] initWithString:deckName];
