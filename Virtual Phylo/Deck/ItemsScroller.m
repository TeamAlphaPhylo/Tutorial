...
+(id)itemsScrollerWithItems:(NSArray *)items andOrientation:(ItemsScrollerOrientations)orientation andRect:(CGRect)rect{
    return [[self alloc] initWithItems:items andOrientation:(ItemsScrollerOrientations)orientation andRect:rect];
}
...
-(void)updateItems:(NSArray*)items{
    ...
    for (CCLayer *item in items)
    {
        if(i == 0){
            ...
            if(_orientation == ItemsScrollerHorizontal){
                csWidth = items.count*_itemSize.width;
                csHeight = _rect.size.height;
            }
            
            if(_orientation == ItemsScrollerVertical){
